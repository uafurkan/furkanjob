    ...
    <div className="stack gap-6">
      ...
      <section className="glass card stack gap-4">
        ...
        <div className="row gap-6 wrap">
          ...
          <label className="toggle"><input type="checkbox" checked={reminderOptOut} onChange={(e) => setReminderOptOut(e.target.checked)} /> {t("pf.reminderOptOut")}</label>
        </div>
          ...
          <div className="stack gap-2">
            {cvs.map((c) => (
              <div key={c.id || c.filename} className="doc-row">
                ...
                {c.id && (
                  <a
                    href={`/api/cv/download?id=${encodeURIComponent(c.id)}`}
                    download={c.filename}
                    className="doc-del"
                    aria-label="Download"
                    title="Download"
