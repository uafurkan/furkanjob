  ...
  return (
    <main className="legal-page">
      ...
      <article className="legal container">
        ...
        <p className="legal-updated">{tr ? "Son güncelleme" : "Last updated"}: {UPDATED}</p>
        ...
        <p className="text-secondary">
          {tr
            ? <>Sorularınız için: <a href={`mailto:${CONTACT}`}>{CONTACT}</a>. Ayrıca <Link href="/privacy">Gizlilik Politikası</Link>'na bakın.</>
            : <>Questions? <a href={`mailto:${CONTACT}`}>{CONTACT}</a>. See also our <Link href="/privacy">Privacy Policy</Link>.</>}
        </p>
        <p><Link href="/" className="btn btn-sm">{tr ? "← Ana sayfa" : "← Home"}</Link></p>
      </article>
    </main>
  );
}
...
function EN() {
  return (
    <>
      ...
      <h2>3. Sending from your account</h2>
      <p>
...
function TR() {
  return (
    <>
      ...
      <h2>3. Kendi hesabınızdan gönderim</h2>
      <p>
      ...
      <h2>4. AI ile üretilen içerik</h2>
      <p>
      ...
      <h2>6. Erişilebilirlik &amp; değişiklikler</h2>
      <p>
